...
rom.flag_segments_resolution_bits = 4
...
back_pal_file = os.path.join(rom.build_dir,"background_pal.png")
...
rom_filename = os.path.join(rom.build_dir, rom_name+".gwm")

# Final ROM file (compressed)
final_rom_filename = os.path.join(output_dir,str(rom.mame_fullname+".gwm"))

### Files to build
BGD_FILE = os.path.join(rom.build_dir,"gnw_background")
BGD_FILE_PAL = os.path.join(rom.build_dir,"gnw_background_pal")
SGD_FILE = os.path.join(rom.build_dir,"gnw_segments")
#SGD_FILE_PAL = os.path.join(rom.build_dir,"gnw_segments_pal")
...
PRW_FILE = os.path.join(rom.build_dir,str("gnw_preview"))
PRW_FILE_PAL = os.path.join(rom.build_dir,str("gnw_preview_pal"))
#PRW_FILE = PRW_FILE.replace(':','')
...
if os.path.isfile(background_file) :
  ...
  if rom.keep_aspect_ratio:
    ...
    #alpha_composite = alpha_composite.resize((bgnd_width,bgnd_height),Image.BICUBIC)
    #alpha_composite = alpha_composite.resize((bgnd_width,bgnd_height),Image.BILINEAR)
    #alpha_composite = alpha_composite.resize((bgnd_width,bgnd_height),Image.BOX)
    alpha_composite = alpha_composite.resize((bgnd_width,bgnd_height),Image.HAMMING)
    #alpha_composite = alpha_composite.resize((bgnd_width,bgnd_height),Image.LANCZOS)
    #alpha_composite = alpha_composite.resize((bgnd_width,bgnd_height),Image.NEAREST)
    
    #brightness_factor = 1.55  # Increase brightness by 50%
    enhancer = ImageEnhance.Brightness(alpha_composite)
    alpha_composite = enhancer.enhance(rom.brightness_factor)
...
######################################################################################
  if rom.background_resolution>0:
    ...
    imP = alpha_composite.convert('RGB').convert('P', palette=Image.ADAPTIVE, colors=50)
    imP.save(back_pal_file)      
    pixels = list(imP.getdata())
    palette = imP.getpalette()
    with open(BGD_FILE_PAL, 'wb') as f:
...
title_png_file = os.path.join(rom.build_dir,rom.name+"_prev.png")
...
title_image = preview.copy().resize((1280,960))
title_fw_image = preview.copy().resize((184,138))

#define LOGO_SMALL_PIX_WIDTH (184)
#define LOGO_SMALL_PIX_HEIGHT (103)
#unit= Image.open("./custom/unit_x206y99_murm.png").convert('RGB')
...
title_fw_image.save(title_png_file)

genprv = Image.open(title_png_file).convert('RGB')

if rom.background_resolution>0:
  pixels = list(genprv.getdata())
  with open(PRW_FILE, 'wb') as f:
    for pix in pixels:
  ...
  imP = genprv.convert('RGB').convert('P', palette=Image.ADAPTIVE, colors=200)
  pixels = list(imP.getdata())
  palette = imP.getpalette()
  with open(PRW_FILE_PAL, 'wb') as f:
    for color in palette:
      f.write(bytes([color]))
  pixels = list(imP.getdata())
  with open(PRW_FILE, 'wb') as f:
...
# 13 types of 2 elements (x 4 bytes)    	(104 bytes)
element_file=[BGD_FILE_PAL, BGD_FILE, SGD_FILE, SGO_FILE, SGX_FILE, SGY_FILE, SGH_FILE, SGW_FILE, MLD_FILE, PGM_FILE, BTN_FILE, PRW_FILE_PAL, PRW_FILE]
...
elif COMPRESS_WITH == NONE_COMPRESSOR:
  compressed_rom = Path(rom_filename).read_bytes()
